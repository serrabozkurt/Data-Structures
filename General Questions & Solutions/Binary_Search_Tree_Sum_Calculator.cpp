...
BTnode* mergeBST(BTnode *bt1, BTnode *bt2){
	...
	if(!bt1 && !bt2){
		...
		BTnode *thiss = new BTnode;
		BTnode *one, *two;
		
	    if(bt1 && !bt2){
			thiss->data = bt1->data;
			thiss->left = mergeBST(bt1->left, NULL);
			thiss->right = mergeBST(bt1->right, NULL);
		}
		else if(!bt1 && bt2){
			thiss->data = bt2->data;
			thiss->left = mergeBST(bt2->left, NULL);
			thiss->right = mergeBST(bt2->right, NULL);
		}
		else{
			thiss->data = (bt1->data + bt2->data);
			thiss->left = mergeBST(bt1->left, bt2->left);
			thiss->right = mergeBST(bt1->right, bt2->right);
		}
		return thiss;
	}
}
